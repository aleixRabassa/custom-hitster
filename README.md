...
> **Status: playable end to end. Phases 0–7 complete** — 499 tests across 36 files. Paste a link or pick one of nine suggested playlists, and you get a shuffled deck you can flip, swipe, scan and play audio from, with a HUD, an exit confirmation and an end screen. The deck resumes across a reload. Phase 8 (card visual design, shareable deck URL, PWA, printable export) is explicitly out of v1 — see [`docs/plans/plan.md`](./docs/plans/plan.md).
>
...
| [`docs/plans/`](./docs/plans/)                       | Per-phase plans: Phase 1, Phase 2 (×2), Phase 3, Phases 4–6 (×3), Phase 7 (×2)                            |
...
- **Touch gestures have never been tested on a real device.** The five thresholds in `src/game/gestures.ts` are documented guesses. If touch input misbehaves, those constants are the first place to look.
