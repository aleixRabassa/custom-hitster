...
- [ ] **Step 1 — Create `src/game/deck-merge.ts`: the merge, the label and the cap.** One pure module
  ...
  - [ ] Export `MAX_DECK_PLAYLISTS` as 5. It is a client-side input rule, so it lives here rather
  ...
  - [ ] Export a `MergedDeck` interface: the ordered `playlists` that loaded, the merged `cards`,
  ...
  - [ ] Export `mergePlaylists(outcomes)`, taking the outcomes **in the order the player entered the
  ...
  - [ ] Dedupe by `Card.id`, first occurrence wins, preserving order. Record in the module header WHY
  ...
  - [ ] When **no** playlist loaded, return the FIRST failure's code, so the landing screen's single
  ...
  - [ ] Guard the degenerate cases explicitly: an empty outcome list, and every playlist loading but
  ...
  - [ ] Export `deckLabel(playlists)`: the first playlist's name for one playlist, and `"<first> +N
...
- [ ] **Step 2 — Widen `GameState` and the `START` action in `src/game/types.ts`.** Replace
  ...
  - [ ] Say in the field's doc comment that the array is ordered as the player entered the rows, that
  ...
  - [ ] Leave `Card`, `PlaylistSummary` and everything in `shared/types.ts` untouched. Nothing about
        the server contract changes in this task, which is the point of the client fan-out.

- [ ] **Step 3 — Update `src/game/reducer.ts`.** Mechanical, and it must stay mechanical: no merge
      logic enters the reducer.
  - [ ] `initialGameState.playlists` is an empty array.
  - [ ] `START`'s two exits (the empty-deck `ended` branch and the normal one) write
  ...
  - [ ] `RESUME` writes `session.playlists`.
  - [ ] Confirm no other action reads the field, so `FLIP`, `NEXT`, `END` and `YEAR_RESOLVED` need no
        edit.

- [ ] **Step 4 — Bump the session format in `src/game/persistence.ts`, and read v1.**
  - [ ] `SESSION_VERSION` becomes 2 and `PersistedSession.playlist` becomes `playlists`.
  - [ ] `toPersistedSession` guards on `state.playlists.length === 0` instead of the null check.
  - [ ] `validateSession` accepts **version 2** natively and **version 1** by lifting its single
  ...
  - [ ] Validate `playlists` as a **non-empty array** of summaries, reusing the existing per-summary
  ...
  - [ ] Keep every other contract untouched: a read failure is a miss, a write failure is a no-op,
        nothing throws, and validation rebuilds field by field rather than casting.

- [ ] **Step 5 — Make the share link carry 1..5 ids in `src/game/deck-link.ts`.**
  - [ ] `DeckLink.playlistId` becomes `playlistIds: string[]`, ordered and deduped.
  - [ ] Parse the canonical form — one `playlist` param holding a comma-separated list — and also
  ...
  - [ ] Run **every** element through `parsePlaylistUrl` exactly as today, so a bare id, a full URL
  ...
  - [ ] Reject — return `null`, which means the plain landing screen with **no error** — for: no
  ...
  - [ ] Dedupe ids before the cap check, so a link that repeats one id is not punished for it.
  - [ ] `buildDeckLink(origin, playlistIds, seed)` joins with a literal comma. A comma is a legal
  ...
  - [ ] Update the header block: the promise is still "same playlists, same shuffle", never "the same
...
- [ ] **Step 6 — Make a library entry hold 1..5 ids in `src/game/playlist-library.ts`.**
  - [ ] `SavedPlaylist.id` becomes `ids: string[]`; `name` becomes the `deckLabel()` output the
        container passes in; `savedAt` is unchanged.
  - [ ] Export `savedDeckKey(entry)` — the entry's ids **sorted and joined** — as the dedupe key, the
  ...
  - [ ] `savePlaylist` dedupes on `savedDeckKey`, keeps most-recent-first, and keeps
  ...
  - [ ] `removePlaylist(storage, key)` takes the deck key rather than a single id.
  - [ ] `LIBRARY_VERSION` becomes 2, and `validateLibrary` accepts version 1 by lifting each entry's
  ...
  - [ ] Validate `ids` as a non-empty array of non-empty strings, cap it at `MAX_DECK_PLAYLISTS` on
  ...
  - [ ] Keep the header block's leak rule verbatim and extend it: an entry is now up to five ids and
        one label, still playlist-level only, still read on a pre-start surface.

- [ ] **Step 7 — Update the `start` signature in `src/game/use-game-session.ts`.** `start(cards,
...
- [ ] **Step 8 — Run the four checks and record the decisions.** `pnpm typecheck && pnpm lint && pnpm
      test && pnpm build`, all four green, before this plan is considered done — plan 2 starts from a
      clean tree.
  - [ ] Grep for `state.playlist` and `.playlist?.` across `src/` and confirm every remaining hit is
...
- [ ] `should merge two loaded playlists into one deck in row order` — covers `mergePlaylists`
- [ ] `should drop a track that appears in two playlists` — covers the dedupe, asserting the card
      count and that the surviving copy is the first one
- [ ] `should keep two different tracks that share a title` — covers that the dedupe key is the id
- [ ] `should report truncated when any playlist truncated` — covers the OR aggregation
- [ ] `should sum skippedCount across playlists` — covers the additive aggregation
- [ ] `should list the failures of the playlists that did not load` — covers partial failure with at
      least one success
- [ ] `should return the first failure code when no playlist loaded` — covers the total-failure exit
      and its ordering
- [ ] `should return empty-playlist when every playlist loaded empty` — covers the guard that keeps an
      empty deck away from `START`
- [ ] `should label a single playlist with its own name` — covers `deckLabel` at n=1
- [ ] `should label three playlists as the first plus a count` — covers `deckLabel` beyond n=1

`src/game/reducer.test.ts` — modified:

- [ ] `should carry every playlist into state on START` — covers the widened action
- [ ] `should replace the playlists wholesale when START runs mid-game` — covers that a new set does
      not merge into the old one
- [ ] `should restore every playlist on RESUME` — covers the resumed array

`src/game/persistence.test.ts` — modified:

- [ ] `should round-trip a session with three playlists` — covers the v2 format end to end
- [ ] `should read a v1 payload by lifting its single playlist` — covers the migration
- [ ] `should reject a payload whose playlists array is empty` — covers the non-empty rule
- [ ] `should reject a payload whose playlists array holds a malformed summary` — covers per-entry
      validation
- [ ] `should not cap the playlists on read` — covers the deliberate asymmetry with the library, so a
      later cap change cannot silently drop a saved game

`src/game/deck-link.test.ts` — modified:

- [ ] `should parse a comma-separated list of ids` — covers the canonical multi form
- [ ] `should still parse a single-id link` — covers back-compat with every link already shared
- [ ] `should parse repeated playlist params` — covers the `getAll` tolerance
- [ ] `should accept full playlist URLs inside the list` — covers that `parsePlaylistUrl` runs per
      element
- [ ] `should reject a link whose list holds an album link` — covers one bad element failing the whole
      link
- [ ] `should reject a link with more than the maximum number of playlists` — covers the cap as a
      rejection rather than a truncation
- [ ] `should dedupe repeated ids before the cap check` — covers the ordering of those two rules
- [ ] `should build a link joining the ids with commas` — covers `buildDeckLink`
- [ ] `should round-trip a built multi link back through the parser` — covers the pair together

`src/game/playlist-library.test.ts` — modified:

- [ ] `should save an entry holding several ids` — covers the widened shape
- [ ] `should dedupe an entry whose ids match an existing one in a different order` — covers
      `savedDeckKey`'s sort
- [ ] `should remove an entry by its deck key` — covers the new remove argument
- [ ] `should read a v1 library by lifting each entry's id` — covers the migration
- [ ] `should reject an entry whose ids array is empty` — covers the validator
- [ ] `should cap the ids of a stored entry on read` — covers the read-side cap
- [ ] `should not write any field beyond ids, name and savedAt` — the existing leak test, re-pointed
      at the array shape
...
- [ ] `docs/architecture.md` §3 — a subsection for the combined deck: the client fan-out, the merge
      module's place in `src/game/`, why `api/` is untouched, and the two storage version bumps with
      their v1 lifts.
- [ ] `docs/architecture.md` §2 — the `src/game/` inventory gains `deck-merge.ts`, described as a
      decision module with no I/O.
- [ ] `AGENTS.md` — one new bullet in the same voice as the existing ones: a deck is now **1..5
...
- [ ] `docs/plans/plan.md` §5 — record multi-playlist as a post-Phase-8 item with a pointer to both
...
- [ ] `docs/agent_findings.md` — a dated (2026-08-07) entry per decision below, plus anything measured
      while building (in particular: what a five-playlist deck actually merges to after the dedupe).
- [ ] Module header blocks — the reasoning lives next to the code as it does everywhere in this repo:
...
- [ ] `deckLabel`'s `"+N more"` suffix goes through `sanitizeForPdf` into the PDF **filename**. Confirm
