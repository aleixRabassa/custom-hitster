...
| `visual-assets/listing.md`                           | Modified | §1's "blocked on one undecided trademark question" and "any screenshot of the picker is blocked" become history once step 3 deploys.                                                |
| `visual-assets/tester-notes.md`                      | New      | The tester notes plan 1 said things "go in" and nothing ever produced.                                                                                                              |
...
- [x] **Step 1 `[Console]` `[you type it]` — Day 0: start both long waits before anything else.**
  ...
  - [x] `[you type it]` **Obtain an Android device** — no step in either old plan does this. Plan 1
        step 9 says "a real device"; `visual-assets/listing.md` §4 allows an emulator for screenshots
...
- [x] **Step 3 `[agent]` — The trademark pass, ending in a deploy. Gate M1.** Decided 2026-09-19:
  ...
  - [x] Reword `public/robots.txt` line 1 so it no longer opens "Custom Hitster is…" — the file is
        ...
        changes.
  - [x] Rewrite `visual-assets/listing.md` §1: the "blocked on one undecided trademark question" status
...
- [x] **Step 7 `[you type it]` — First build, first install, URL bar expected. Gate M2a.** Absorbs
      ...
      true and the conclusion was wrong. **The box now waits on a DECISION, not an observation** — and
      ...
      stands**, and step 10 uses it. Written into `visual-assets/tester-notes.md` under "expected", since
  ...
  - [x] Run plan 2's rows 1–6 ("The Android back press"); **attempt row 7** (predictive back) too. A
        ...
        `visual-assets/tester-notes.md` as expected behaviour._
...
- [x] **Step 8 `[agent]` — Write the tester notes.** Plan 1 step 9 and its decision 19 say the
      ...
      `visual-assets/tester-notes.md` with: how to opt in to the closed track; that the app shares its
...
- [ ] **Step 9 `[Console]` — Create the Play Console app.** Absorbs plan 1 step 10, second half;
      ...
      Games category with a music/trivia subcategory, **PAID at €1.00 one-time** (changed 2026-09-21,
      decision 16), not primarily child-directed.
      **UNBLOCKED 2026-09-21** — identity verification and every profile check completed, one day
      inside the wait started 2026-09-20. Values are `visual-assets/listing.md` §5; three notes on which
      of them are hard to undo. **Free is the one that does not go back, which is why Paid is the safe
      answer to give here**: Google allows paid → free and refuses free → paid outright — an app once
      offered free can only be charged for as a NEW app with a NEW package name, which would discard
      `aleixrabassa.playlistjitster`. Answering Paid keeps both options; answering Free would close one
      permanently. **The package id is NOT typed here** — the Console binds it from the first uploaded
      bundle at step 10, which is where the permanence rule in `AGENTS.md` actually bites; there is no
      field to get wrong at creation. And **"App or game" answers Game**, which is what makes the Games
      category reachable at all. Category, subcategory, name and default language are all editable
      afterwards. **The price itself is NOT set here**: it needs a payments profile first (new step
      9a), and App pricing is a separate page.

- [ ] **Step 9a `[Console]` — Set up the payments profile. START IT BEFORE STEP 9, NOT AFTER.** New
      2026-09-21, created by decision 16 and by nothing in the original plan — which assumed a free app
      and therefore never needed a merchant account. Google's own order is "set up a payments profile →
      review the price ranges → enter a price", and a payments profile carries bank details, tax
      residency and its own verification cycle, so it is **a calendar wait of the same kind step 1
      existed to start early**. It does not gate app creation, so step 9 proceeds in parallel; it gates
      the App pricing page, and therefore any release that charges. Record the start date in
      `docs/agent_findings.md` exactly as step 1 did. Then check **€1.00 against Play's price-range
      list for EUR** — the minimum was deliberately not guessed — and read off the profile what the €1
      actually nets after Play's service fee and after whatever VAT treatment applies in Spain, which
...
- [ ] **Step 13 `[you type it]` `[Console]` — Build the store listing.** Absorbs plan 1 step 13; the
      copy already exists in `visual-assets/listing.md`.
  - [x] Compose the 1024×500 feature graphic from `visual-assets/logo-master/logo.png` (the master; never anything
  ...
  - [x] Phone screenshots: the **welcome screen first** (a cold launch shows it), then the picker
        (unblocked since step 3), a dealt card, a revealed card.
        _2026-09-24: both produced, plus tablet, Chromebook and social formats, in
        `visual-assets/assets/` — inventory in `listing.md` §4a. The 512 icon was opened and found to be
        a palette PNG, so `assets/graphics/play-icon-512.png` is the one to upload. Upload is the
        open box below._
...
- [ ] **Step 16 `[you type it]` `[Console]` — Closed testing. Gate M3, then a fourteen-day wait.**
      ...
      testers** (a step plan 1 assumed and never wrote), send them `visual-assets/tester-notes.md`, and
...
- [ ] `visual-assets/listing.md` — §1 rewritten after the relabel (step 3); the assets checklist ticked
...
- [x] `visual-assets/tester-notes.md` — new, a deliverable in its own right (step 8).
...
| 10  | The tester notes are a committed file under `visual-assets/`, not a message typed into the Console.             | Plan 1 said two things "go in the tester notes" and nothing produced them. Twelve people read it; it should be reviewable and reused for the next track.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 16  | The app is **PAID, €1.00, one-time**, reversing this plan's "free" throughout.                                        | Developer's choice on 2026-09-21. The order is the safe one: Google allows paid → free and refuses free → paid except as a new app with a new package name, so answering Paid keeps both doors open while Free would shut one on `aleixrabassa.playlistjitster`. Costs: a payments profile becomes a prerequisite (step 9a), and the twelve testers need a route that does not charge them (Open Questions). **Raised once and reaffirmed**: the same game is free and public at the pinned origin, which a paid listing's reviewers will compare, and a paid thin wrapper of a free site sits closer to Play's minimum-functionality scrutiny than a free one. Recorded, not re-argued. |
...
- [x] Which rows may an emulator satisfy, if no physical device is available at step 1? The doubtful
      ...
      remains permissible only for screenshots (`visual-assets/listing.md` §4). Re-open only if the
...
- [ ] **How do the twelve testers get the app without paying €1?** _→ **Partly answered
      ...
      tester the €1 by hand. Still blocks rewriting step 3 of `visual-assets/tester-notes.md`._
      ...
      `visual-assets/tester-notes.md`**, whose "How to join" currently says "install it as you would any
...
- [x] Must Vercel's access-log IP retention be declared in the Data safety form, or does it fall under
      ...
      by the developer** — the conservative answer, and recorded in `visual-assets/listing.md` §5._
