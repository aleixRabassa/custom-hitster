...
| `shared/__fixtures__/year-candidates.ts` | New      | **Not in the original plan.** Normalized `RecordingCandidate[]` for the 14 known-tricky tracks, feeding the accuracy suite. Split out from `api/_lib/__fixtures__/` because `tsconfig.app.json` covers `src` + `shared` only, so a `shared/` test importing from `api/` would drag Node-side code into the browser typecheck |
...
| `api/_lib/resolve-year.ts`               | New      | **Not in the original plan.** The orchestration the plan assigned to `api/year.ts`: clean, read cache, gate, strict, relaxed, write cache. Extracted so the orderings the plan cares about are testable rather than prose — see Execution Notes                                                                              |
| `api/_lib/resolve-year.test.ts`          | New      | Those orderings, including `should not consume a permit when the caller had a cache hit`                                                                                                                                                                                                                                     |
...
| `.env.example`                           | Modified | No new variables; comments corrected now that the code reads them (see Documentation Updates)                                                                                                                                                                                                                                |
| `AGENTS.md`                              | Modified | **Not in the original plan.** Phase status moved to "Phase 2 complete, Phase 3 next", and both phase-2 plan files added to the documentation index                                                                                                                                                                           |
| `.gitignore`                             | Modified | **Incidental, not this plan's work.** A bare `.env` was not ignored, and `vercel link` later appended a `.env*` that shadowed `!.env.example`. Both fixed while verifying; recorded here only so the diff is not a surprise                                                                                                  |
...
> **Read the Execution Notes before trusting the two paragraphs below.** They describe the design as
> planned, and execution changed it in three measured ways: the year comes from the release **group's**
> `first-release-date` rather than the earliest release date (the latter is the reissue date and is wrong
> by decades), `limit=100` is load-bearing rather than "modest", and duration is spent mainly as a `dur:`
> bound on the query rather than as a local tie-breaker. The tiering and the filters themselves survived
> unchanged.
...
- **Manual verification — run through `vercel dev` on 2026-08-04, outcomes recorded.** Three items on
  the original list turned out to be **unachievable through `vercel dev`**, because it runs a fresh
  process per invocation and adds ~4 s to every request; that is a property of the dev server, not a
  defect in the code, and it is why they are marked blocked rather than failed. Full measurement in
  [`agent_findings.md`](../agent_findings.md).

  | Check                                                                                       | Result                                                                                                                        |
  | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
  | Phase 0 tricky tracks resolve to their known-correct years                                  | **PASS** — Billie Jean 1982 `high`. Also 8/8 through the resolver in-process, all `high`                                      |
  | A remaster suffix is stripped and the track still resolves                                  | **PASS** — `Bohemian Rhapsody - Remastered 2011` → 1975, `cleanedTitle: "Bohemian Rhapsody"`                                  |
  | A nonsensical title gives `year: null`, `confidence: 'none'`, and a reason                  | **PASS** — `reason: 'no-candidates'`, not a wrong year and not a 500                                                          |
  | An unset `MUSICBRAINZ_USER_AGENT` gives a 500 with an explicit message                      | **PASS** — `500 not-configured`, on a request that would otherwise have hit the cache                                         |
  | Wrong verb gives 405 with an `Allow` header                                                 | **PASS**                                                                                                                      |
  | No Upstash credentials: the in-memory adapter is selected and logged, lookups still succeed | **PASS** on selection and logging, but see below — "still succeed" is true while "still cache" is not                         |
  | `/api/hello` still returns `maxEmbedTracks: 100`                                            | **PASS**                                                                                                                      |
  | A repeat request reports `cached: true`                                                     | **BLOCKED** — needs Upstash. Nothing survives between invocations locally, so `cached` is always false. Covered by unit tests |
  | Rapid requests give a 429 with `retryAfterMs`                                               | **BLOCKED** — same cause: a fresh gate admits every request. Five rapid calls returned five 200s. Covered by unit tests       |
  | Wall-clock cost of a real 50-track playlist                                                 | **NOT MEASURED, and not measurable this way** — ~4 s of dev-server overhead per request dominates it. Needs a deployment      |

  Two things follow for whoever picks this up:

  - **Configure Upstash before resolving a whole deck locally.** Without it the gate paces nothing, so
    local development sends MusicBrainz unpaced traffic — two requests per lookup against a limit they
    enforce by blocking.
  - **The 50-track number is still owed to Phase 3.** Either take the in-process figure of **1.3-3.6 s per
    cold track** (gate and both round trips included, function overhead excluded) or run the harness
    against a preview deployment with Upstash configured. Note it is slower than the "one track per two
    seconds" decision 19 assumed.

  Still outstanding, and deliberately out of scope here: after deploying, confirm `/api/hello` still
  returns `maxEmbedTracks: 100` **from production** and that `/api/_lib/musicbrainz` is not a live route.
...
| 2   | **The strict filter is Phase 0's verified fix — the FILTERS unchanged, but the date source, page size and duration handling all corrected during execution (see Execution Notes):** release-group `primary-type: Album`, no Live/Compilation/Remix/Bootleg secondary types, release `status: Official`, duration tie-break | Measured, not guessed: a naive top-scored lookup was ~6% accurate (1 of 18), while this filter was correct in all 12 cases tried — every compilation-era track, every live-version track, and all six cover-versus-original lookups. Reimplementing it differently means re-earning that evidence.                                                                                                                                                                                   |
