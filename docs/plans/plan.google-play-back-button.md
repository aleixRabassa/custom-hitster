...
and worse, it bypasses it *invisibly*, because the session survives in `localStorage` and a relaunch
...
it does not make back a silent exit: back becomes a *request*, exactly like the Exit button, so the
...
- [x] **Step 1 — Write the decision module.** `src/game/back-navigation.ts` exports a function
  ...
  - [x] Make the function **total** and fix the precedence for the case where both flags read open.
        ...
        precedence deliberately costs one line. *Precedence is DOM order: the deck-actions panel is
        rendered last in `GameScreen`, so with both mounted it is the one painting on top, and back
        dismisses what the player can see.*
  - [x] Keep it free of React, of the DOM and of any history API. It takes plain data and returns a
        plain value.
  - [x] Document in the file header *why* back is a request rather than an exit: ending the game is
...
- [x] **Step 2 — Write the binding hook.** `src/hooks/useBackNavigation.ts` takes the dialog state
  ...
  - [x] Push an entry that **does not change the visible URL** and does not discard the current query
        ...
        mid-game. *`pushState(state, '')` with no URL argument — measured to keep the href, query and
        fragment in full.*
  - [x] **Handle the StrictMode double-mount.** The effect runs twice in development, so a naive
        ...
        hook's own test, rather than reasoning about it. *An undoing cleanup turned out to be
        ...
        count of traversals this hook itself queued — is what closes it.*
  ...
  - [x] Decide and document what happens if the entry cannot be cleanly removed. The failure mode to
        ...
        visible instead of closing the app, which reads as a frozen back button. *A push that throws
        reports itself: `hasEntry` stays false, the cleanup navigates nowhere and leaves no stray,
        and back degrades to exactly the platform behaviour it has today.*
...
- [x] **Step 3 — Wire it into `GameScreen`.** Call the hook, passing `isExitConfirmOpen`,
  ...
  - [x] Confirm **guard 4 needs no change.** It gates the window key handler on the two dialog flags
        ...
        that guard exists because → would otherwise deal a card behind a backdrop. *Verified: back
        either closes one of those two dialogs or opens the exit confirmation, so every state that
        puts a backdrop over the card is still one of the two the OR already reads.*
  - [x] Add a note to the component header explaining that the hook's mounting *is* the scoping, so a
...
- [x] **Step 4 — Accept and record the browser-side consequence.** This is not TWA-only: it also
      ...
      than a side effect. *Accepted. Recorded in `useBackNavigation.ts`'s header, which also states
      the two things not to add later: a user-agent sniff and a `display-mode: standalone` check.*
...
- [ ] `should request an exit when no dialog is open` — covers the decision in
      `src/game/back-navigation.test.ts`. The core case: back during play must produce a *request*,
      never a direct exit.
- [ ] `should close the deck-actions dialog when it is open` — covers the decision in
      `src/game/back-navigation.test.ts`.
- [ ] `should close the exit confirmation when it is open` — covers the decision in
      ...
      interception at all.
- [ ] `should resolve a defined action for every state combination` — covers totality and the chosen
      precedence in `src/game/back-navigation.test.ts`.
- [ ] `should push exactly one history entry on mount` — covers the binding in
      `src/hooks/useBackNavigation.test.ts`, including under a StrictMode double-mount.
- [ ] `should leave the history length unchanged after unmount` — covers cleanup in
      `src/hooks/useBackNavigation.test.ts`. This is the assertion that catches the stray-entry
      failure, which on a device presents as a back button that does nothing.
- [ ] `should not run the decision when cleanup removes its own entry` — covers the listener-before-
      navigation ordering in `src/hooks/useBackNavigation.test.ts`. The hazard is real and the
      symptom is a dialog opening on an unmounting screen.
- [ ] `should preserve the query string` — covers the pushed entry in
      `src/hooks/useBackNavigation.test.ts`, so a mid-game reload of a shared link still works.
- [ ] `should open the exit confirmation rather than calling onExit on a back press` — covers the
      wiring in `src/components/GameScreen.test.tsx`. The end-to-end assertion that back is a request.
- [ ] `should not intercept a back press outside the game screen` — covers the mounting-is-scoping
      claim, in `src/components/GameScreen.test.tsx` or `src/App.test.tsx`, by asserting no entry is
      pushed for the landing and end screens.
...
- **Manual verification:** step 5 in full, on the installed TWA. Nothing in this plan is
  *confirmed* by a green test run; the tests guard the logic, the device confirms the behaviour.
...
| 5   | Back inside a dialog closes that dialog and nothing else.                              | It is what every Android app does, and a back press that *confirmed* an exit would be worse than no interception.                                                                              |
...
- [ ] Does jsdom's `history.back()` fire `popstate`, and is the cleanup ordering observable there? It
      decides how much the hook's tests actually prove — record the answer either way.
