...
- [x] **Add the hello-world Vercel function at `api/hello.ts`** — typed with `@vercel/node`'s request/response types, returning a small JSON payload. Its purpose is to establish the handler signature, the import style, and the tsconfig membership that Phase 2's `/api/playlist` and `/api/year` will copy.
  ...
  > for consistency. Type-checks under `tsconfig.api.json`.
  >
  > **Deploy confirmed 2026-08-03** — the developer linked and deployed successfully, so the
  > extensionless relative `shared/` import survives the real Vercel function build, and the
  > no-project-references root `tsconfig.json` (decision 14) is validated in production rather
  > than only in theory. This is the pattern Phase 2's `/api/playlist` and `/api/year` should copy
  > verbatim.
...
| 4   | **Vercel configuration is committed, but no Vercel CLI command is run as part of this phase**                                                                                                                                                                          | Developer will perform linking and deployment themselves after implementation completes. **Closed out 2026-08-03: the developer linked and deployed successfully**, so the `plan.md` Vercel checkbox is now ticked outright and the "hello-world function deploys" half is no longer unproven.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- [ ] **Should the `@/` alias exist at all, given `api/` cannot use it?** Decided yes, app-scoped — but if the asymmetry causes confusion in Phase 2, dropping it in favour of relative imports everywhere is a cheap reversal while the codebase is still this small. Still open as a _design_ question, but no longer a risk: the 2026-08-03 deploy succeeded with `api/` importing `shared/` relatively (grep confirmed no aliased import under `api/` beforehand), so the asymmetry is proven to work. Note the deploy validates the **relative** side only — nobody has yet observed what an aliased import from `api/` actually does on Vercel, and per Vercel's docs it should not be tried.
