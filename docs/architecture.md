...
**The wait offers "Print so far" beside its Cancel** (2026-08-07), and that is not a hole in the gate. The gate refuses a deck that is **quietly** short — it does not claim a short deck is never wanted, and somebody who wants to start playing with 40 of 60 cards is making a trade. What makes it a trade rather than a surprise is the excluded **count** after the press — "PDF downloaded — N cards left out, no year yet". An explanatory caption above the buttons was written first and then cut: the button's own label says what it prints, and a sentence repeating it is noise in a panel that already carries two. The consequence is that the excluded count is the **whole** of the disclosure, so it is the line not to weaken. The press deliberately leaves `hasAskedToPrint` alone, so the wait survives its own export and the full deck still exports itself when the last year lands: **two files, both asked for.** Two structural consequences. `role="status"` moved from the wait's outer element onto the two sentences inside it — the new button's label counts QR codes as they are generated, and a count climbing inside a live region is one announcement per card, so the buttons and the export's outcome sit outside the region (the outcome brings the separate region it already had in the resolved view, which is the same arrangement as down there). And the outcome paragraph became `ExportMessage`, shared by both views, so one `PdfExportState` cannot be described in two different sets of words. Focus stays on **Cancel** rather than moving to the new first action, against the dialog's own first-action convention, because an Enter pressed reflexively on arrival should not spend paper.

Making a finished export assertable meant doubling `qrcode` **and** `jspdf` in `DeckActions.test.tsx`: the real QR build draws through a `<canvas>` jsdom does not implement and jsPDF hands a download to a browser that is not there, which is why every export test written before this stopped at `nothing-to-print` — the one outcome reached before either dynamic `import()` runs.
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/game/gestures.ts`         | `shouldCommitSwipe`, `swipeDirection`, `swipeIntent`, `deckMovementFor`, `isTap`, and 5 named threshold constants. No React                                                                                                           |
...
| `src/components/CardStack.tsx` | Calls the hook, owns `AnimatePresence` and the keys, and derives the exit direction from the index delta into `AnimatePresence custom` (a `useState` latch keyed on the presence key — a ref read in render fails `react-hooks/refs`) |
| `src/components/Card.tsx`      | Spreads `gestureProps` onto its outer `motion.div`; owns its own exit variant (`CARD_VARIANTS`), a dynamic variant that reads `custom`                                                                                                |
...
**A step back PLAYS THE DEAL IN REVERSE, which is why one index delta feeds two Motion channels (2026-09-21).** Until then a step back mirrored the advance — it threw the current card off to the LEFT — and since `AnimatePresence initial={false}` meant nothing ever animated IN, the two directions were indistinguishable on screen. That was the developer's complaint. Forward is unchanged: the outgoing card flies to `+600` and uncovers the card beneath it. Backward reverses the picture instead of mirroring it: **the card being returned to comes back from off the RIGHT edge** (`x: 600 → 0`) and the card being left settles to `x: 0` underneath it. `deckMovementFor(previousIndex, nextIndex)` in `gestures.ts` decides which — `'forward' | 'backward'`, renamed off `CommitDirection` because nothing moves left any more — and `CardStack` latches it on the presence key and hands the one value to both channels. **Two channels, because Motion allows no single one**: the exit must read `AnimatePresence custom` (an exiting child animates with the props of its last render, and a keyboard advance changes the index and removes the card in the same render), while the entrance must read a plain prop (motion-dom passes `presenceContext.custom` to variant resolution only for `type === "exit"`, and framer-motion's `makeLatestValues` resolves the first-paint inline style with no custom at all, so a function `initial` would paint one frame at rest and then jump). The paint order is the other half: `popLayout` absolutises the outgoing card, so it would paint OVER the returning one — the backward exit variant carries `zIndex: -1`, which puts it under in-flow content but still above the `-z-10` preload inside the stack's `isolate`. The forward path is untouched by construction, because the entering card's `animate={{ x: 0 }}` resolves to `transform: none` and this app never registers `MotionGlobalConfig.WillChange`, so it creates no stacking context. None of the z-ordering is observable in jsdom; it is rows 10 and 11 in [`development.md`](./development.md) §5.

**And the step back is PERFORMED BY THE FINGER rather than played on release (2026-09-21, later the same day).** Two developer instructions. The card on top of the deck must never travel left of where it rests, so `dragElastic` went from the scalar `0.35` to `{ top: 0, right: 0.35, bottom: 0, left: 0 }` against the same `left: 0` constraint — Motion clamps at the constraint going left, and rightward travel is untouched. And every pixel of leftward travel past that point moves the **previous** card in from the right, one for one, with the release only finishing what is left. `previousCardProgress(offsetX, cardWidth)` in `gestures.ts` is the whole mapping, there for the reason every threshold is there; it reads Motion's POINTER offset, so "drag right, then slide back left past the origin" engages the previous card by the overshoot without being special-cased. The peek parks **one card-width out**, not at `EXIT_DISTANCE_PX`: a 360px phone renders a 288px card at `p-6`, so a card parked at `x: 600` sits 276px beyond the viewport's right edge while the swipe commits at 96px — the player would see nothing at all move before releasing. 600 stays the **keyboard's** entrance, which has no finger to track. The peek is a `motion.div` in `CardStack` whose `x` and `display` are both `MotionValue`s off one source, so a drag costs React zero renders; `display` specifically, because `opacity` and `visibility` leave the element laid out a card-width to the right of the deck, which is a permanent horizontal scroll on a phone. The card's width is sampled once per gesture from the stack wrapper's `offsetWidth` (the token is a `clamp()`, so no constant is right, and reading it per pointer move would force a layout every frame). **The handoff is the fragile part**: the hook reports `onPrevious(fromProgress)`, `CardStack` records it against the presence key the returned-to card is about to render under, and `Card` spends it as `initial.x` as a percentage of its own width — so the card continues from the thumb rather than jumping back off-screen for a fresh 250 ms. The record is keyed, and spent in the same render-phase guard that latches the movement — cleared on the first card change that is not the one it names. Letting it merely go stale is not enough, because the presence key is the same string every time the deck is on that card: drag back to card 4, ArrowRight, ArrowLeft would otherwise bring card 4 in from a thumb position two moves old. An effect would be the `set-state-in-effect` the lint rejects, and the sequence needs a real drag, so no jsdom test can reach it. Rows 13–15 in [`development.md`](./development.md) §5, all Pending, including the known rough edge: the cancelled peek's slide-out is an imperative `animate()` and so does not read `MotionConfig reducedMotion="user"`.
...
src/components/
  ...
  Spinner.tsx           The one `data-motion="spinner"` element. Hidden, not stopped,
                        under reduced motion — so callers say it all in text too
src/game/
...
**It is a container flag, not a fifth status.** `App.tsx` holds `hasEnteredPicker` in a `useState`, the same shape as `endedView`, seeded as `useState(deckLink !== null || state.status !== 'idle')` — a valid share link, or a restored session, counts as having pressed through; the second term is sound only because `useGameSession` runs `RESUME` in `useReducer`'s lazy initializer, so the first render already carries the restored status — and every branch that shows the picker returns the one `picker` value, `hasEnteredPicker ? landing : welcome`, computed once. The first version guarded the `idle` branch with `deckLink === null` instead; the seed replaced it the same day (see below). What the picker's Back button added is the other end of the flag. **It is set true from three places and cleared from one**: the welcome button sets it, and so do Exit and Home; Back clears it. And **both `ended` branches that show the picker — `deckCollapsed`, and `endedView === 'landing'` — now render the welcome screen instead while the flag is false.** Together those buy the edge cases with no code of their own: **a share link never sees it** (the link's request is already in flight and the picker is where its loading state and error slot live), **a saved session resumes past it** (the seed makes the flag true from the start; a `preparing`/`playing` status also never reaches a check, but a resumed deck that collapses to zero DOES reach the `ended` branch, which is why the seed and not the status is what carries it), and **Exit and Home land on the picker** — a player who just quit a game does not need the rules again — but note the reason moved: they land there because they **set the flag**, not because they go through `ended`; through `ended` alone they would now reach the front door, which is exactly what Back relies on. **No branch consults `deckLink`, and that is what the seed bought.** The `deckLink === null` guard on the `idle` branch had two holes: a link whose fetch FAILED left the picker showing its error with a Back button that did nothing (the flag was already false, the guard still refused), and a link-dealt deck that collapsed to zero showed the front door instead of the `no-years-found` warning. A third outlived the link seed until 2026-09-19: a resumed session that drained to zero (a save taken during the card-1 gate, or a pre-reversal save of nothing but yearless cards) landed on `deckCollapsed` with the flag false and showed the front door with no warning. Seeding the flag from the link and from the restored status says the honest thing — the link, or the earlier visit, pressed through for you — and closes all three. A malformed link is `deckLink === null`, so it gets the plain front door — with no error, as before. The flag is ephemeral: a reload shows the welcome screen again, and a persisted "seen it" flag was deliberately not built.

**What is on it.** The same 192px logo as the picker, as the `<h1>`; one tagline (`COPY.welcome.tagline` — the lead sentence beneath it was cut on 2026-09-18 because the three steps already say it, a one-line change under the copy rule with no test touched); the one big button (`COPY.welcome.enter`, kept distinct from `COPY.landing.start` so no session ever has two buttons called "Start"); "How it works" as an `<ol>` of three step cards with inline-SVG icons in `CardControls`' style; and "Prefer paper?" with a decorative neon card beside the download. Every sentence is `COPY.welcome`. It is a **pre-start surface** and takes no `Card`; its leak proxy reads text AND the audited attributes (`alt`, `aria-label`, `title`, `placeholder`, `value`, `download`, `href` — one list, owned by `src/components/__fixtures__/auditable-text.ts` and shared with `LandingScreen.test.tsx`) and subtracts five strings by exact string: `COPYRIGHT_NOTICE`, `COPY.footer.authorUrl`, `COPY.welcome.printDetail` (which names only the start year), and the two that actually hold the printed range "1970–2033" — `COPY.welcome.yearCardsFileName` (the `download`) and `YEAR_CARDS_PDF_PATH` (the `href`). The test asserts the audit reads both before subtracting them, so the `\b(19|20)\d{2}\b` pattern stays absolute for everything else and cannot pass by omission again (until 2026-09-19 it did: neither attribute was read, so the range escaped the proxy unnoticed).

**The picker has a Back button, and it is a `<button>`.** `LandingScreen` renders `COPY.landing.backToWelcome` as a ghost button `absolute` in the top-left corner of its `relative` `<main>` (out of flow — in the column it cost a 44px row plus `gap-8` above the logo) — an `aria-hidden` `←` glyph beside visible text, `touch-target` and `focus-visible:focus-ring` like every control, and **disabled while a request is loading** — the picker is where the request's loading state and error slot live, so Back waits for it. It takes a required `onBack` prop, and `App.tsx` answers it by clearing `hasEnteredPicker`. It is not an anchor and pushes nothing: there is no router, and the app's one `pushState` stays in `useBackNavigation.ts` for the game screen alone. Consequence to know: returning to the picker from the welcome screen **remounts** `LandingScreen`, so rows typed before pressing Back are gone — accepted, not persisted.

**The PDF is a static asset, and two properties of the shell are load-bearing.** It ships from `public/year-cards-1970-2033.pdf` (10 A4 pages — eight of year cards, 4 × 4 to a page, then two of a repeated card back — ReportLab, re-saved by PDFsharp, supplied by the developer; decoded and measured 2026-09-21, see `agent_findings.md`). `vercel.json`'s SPA rewrite excludes any path containing a dot, so it is served as a file. It is **not precached**: `globPatterns` deliberately has no `pdf`, because 240 kB on every install for a file most players never download is the same class of cost as the logo master this repo keeps out of `public/`. But the worker's SPA fallback would then serve `index.html` for a navigation to it, so `vite.config.ts` adds `/\.pdf(\?|$)/` to `navigateFallbackDenylist` beside `/^\/api\//` — anchored on `(\?|$)` rather than `$` because workbox's `NavigationRoute` tests the denylist against `url.pathname + url.search` (never `url.hash`), so `$` alone stopped matching the moment a query string was appended. The file is also pinned `binary` in `.gitattributes`: it has no NUL in its first 8 kB, so git treated it as text and `core.autocrlf` stripped 23 carriage returns on add, leaving the deployed blob 23 bytes short with a `startxref` pointing past the xref table (lenient readers repaired it silently; the fix reaches production on the next deploy). The link's `download` attribute (its value is `COPY.welcome.yearCardsFileName`, user-visible in a downloads list and therefore copy) is not a defence on its own — whether a download reaches the worker as a navigation differs by browser. Neither half can be observed under any dev server; both are Pending rows in `development.md` §5.
...
**The PDF prints on a light palette regardless of the screen's**, which is the one place the print path deliberately ignores the `@theme` layer. Two reasons, and the second is the real one: ink cost, and **a QR scans as dark modules on a light field with a quiet zone** — inverting or tinting it is how a printed deck fails at the one job the QR has. The geometry is in millimetres and answers to A4 and a pair of scissors, so nothing in CSS could consume it either: **48.9722 mm square cards, 4 × 4 = 16 per sheet**, measured off the app's own `public/year-cards-1970-2033.pdf` on 2026-09-21 so a printed deck and a printed year card are the same object in the hand. That **reverses the 2026-08-06 decision** to use the 65 mm shop-bought card size: the template is a committed file this repo can measure, a boxed game is not, and two card sizes on one table is the failure the size was chosen to avoid. `pdf-sheet.ts`'s header carries the full argument and the template's own `re` operator.

**Front and back sheets are interleaved and the BACK's columns are mirrored.** Long-edge duplex flips the paper about its vertical centre line, so a back sheet laid out in reading order pairs every card with the wrong answer — sixteen cards wrong per sheet, discoverable only after printing and cutting. Because the grid is centred, the correction is exactly a reflection (`xFront + xBack === PAGE_WIDTH_MM - CARD_SIZE_MM`), which is what `pdf-sheet.test.ts` asserts rather than three literal positions. **Short-edge binding would mirror the rows instead and is not supported**: the app cannot read a printer setting, so the docs and the end screen name the setting instead of the code guessing at it.
