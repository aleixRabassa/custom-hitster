...
| #   | Check                                                                                                                                                                                                                                                                            | Status                                                                |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| 1   | **Start a game, press back.** The confirmation appears, the deck is intact, and cancelling returns to the same card.                                                                                                                                                             | Pending — needs the TWA                                               |
| 2   | **Open the deck-actions dialog, press back.** The dialog closes and the game is untouched.                                                                                                                                                                                       | Pending                                                               |
| 3   | **Open the exit confirmation, press back.** It closes without exiting — a back press must never answer the question.                                                                                                                                                             | Pending                                                               |
| 4   | **Back is NOT intercepted on welcome, landing, preparing or end.** One press closes the app. This is the mounting-is-scoping claim on real hardware — and the welcome screen is the front door since 2026-09-18, so it is the first surface a back press meets on a cold launch. | Pending                                                               |
| 5   | **Exit properly, land on the landing screen, press back once → the app closes.** A second press being needed means the cleanup left an entry behind, which presents as a frozen back button.                                                                                     | Pending — the failure this row catches is worse than the original bug |
| 6   | **The edge-swipe gesture as well as the on-screen button.** Same event, different animation.                                                                                                                                                                                     | Pending                                                               |
| 7   | **Android 13+ predictive back.** Confirm the OS does not preview the app peeling away while the web app is in fact handling the press — that looks broken even though nothing is wrong.                                                                                          | Pending — open question; may need a shell-side change in `android/`   |

**Row 7 is the one that could send this back to plan 1.** If the generated shell opts into the legacy
back behaviour, the predictive animation and the intercepted press disagree; the fix would be in the
Android shell rather than in any of this code. Row 1 is the one to run first — it is the whole feature.
...
### The release process — step 1 and the shell generation are RUN; steps 7 to 18 are not
...
3. **Check `targetSdkVersion`** — Bubblewrap 1.25.0 emitted **`targetSdkVersion 36`** with `compileSdkVersion 36` and `minSdkVersion 21` on 2026-09-20, which is the newest API level available; Play's own minimum for new apps still has to be read from the Console policy page on the day, and the bump, if one is ever needed, goes in **`android/twa-manifest.json`** and never in `android/app/build.gradle`, which `bubblewrap update` deletes and regenerates. Check it in the generated Gradle files against Play's current minimum for new apps, which advances every August. Read the Console's own policy page on the day, and read what Bubblewrap actually emitted; do not carry over a remembered number.
