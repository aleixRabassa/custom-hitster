...
| 4   | **Back is NOT intercepted on landing, preparing or end.** One press closes the app. This is the mounting-is-scoping claim on real hardware.                                                  | Pending                                                               |
...
3. **Check `targetSdkVersion`** in the generated Gradle files against Play's current minimum for new apps, which advances every August. Read the Console's own policy page on the day, and read what Bubblewrap actually emitted; do not carry over a remembered number.
