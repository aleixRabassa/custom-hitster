...
| 5   | **Export a real deck, print it double-sided on the LONG edge, and check front/back alignment.** Hold a sheet to a light: the cut outlines must coincide. If they do not, the printer used short-edge binding — which is not supported, and which no code can detect. **Re-run since 2026-09-21**: the grid is now 4 × 4, so a mirror that was right for three columns is wrong for four, and the failure still only shows on paper. | Pending                                                                                                                                                                                                                 |
| 6   | **Scan a printed QR with a phone.** The printed palette is light and the code has a 4.52 mm paper quiet zone plus one module of its own; both exist for this check. **Re-run since 2026-09-21** — the symbol shrank from 53 mm to 39.93 mm with the card, and it is the smaller printed code that decides whether the padding was scaled far enough.                                                                                | Pending                                                                                                                                                                                                                 |
...
| 8   | **Export a 100-card deck** and watch the progress count climb rather than the tab freezing. Seven sheets since 2026-09-21 (16 to a sheet, not 12), ~1200 QR modules per card.                                                                                                                                                                                                                                                       | Pending                                                                                                                                                                                                                 |
...
| 14 | **Cut one deck card and one year card and put them in the same hand (2026-09-21).** This is the whole point of the geometry change: both must measure 48.97 mm square and sit in the same place on their sheets, so the two shuffle together. Print one page of each, cut both, and stack them. Nothing local can check it — the deck's numbers were derived to match the template's measured ones, and only paper says whether the derivation and the printer agree. | Pending |
| 15 | **A worst-case back does not collide or overflow.** Export a deck holding a track whose title wraps to three lines and whose artist wraps to two (a long "(feat. …) - Remastered" title on a multi-artist track). The model says 0.70 mm between the artist's cap height and the last title line's descender and 2.24 mm of slack to the cut; a printer's own hinting is what could eat either. | Pending |
...
| 7   | **Android 13+ predictive back.** Confirm the OS does not preview the app peeling away while the web app is in fact handling the press — that looks broken even though nothing is wrong. _2026-09-21: **not run — and the shell-side worry this row was written around is ANSWERED, from Android's own docs rather than from memory.** The generated `AndroidManifest.xml` sets no `android:enableOnBackInvokedCallback` at all, and the doc's rule is that predictive back is **enabled by default** — the attribute exists to opt OUT (`"false"`), not in. So the shell has NOT kept the legacy behaviour and no change in `android/` is indicated. What remains is purely observational, and HOW to observe it depends on the device's OS: on **Android 15+** the developer option is gone and the system animations appear for opted-in apps automatically; on **Android 13 or 14** they sit behind Settings → System → Developer options → **Predictive back animations**. Read from `developer.android.com/guide/navigation/custom-back/predictive-back-gesture`, 2026-09-21. **The device is Android 15+, confirmed by the developer 2026-09-21** — the number step 1 asked to have recorded and that `adb` never got to read. Two consequences, and the second is the useful one: nothing needs enabling, and **rows 1–6 were therefore run with predictive back already LIVE**, since it is on by default there. The disagreement this row hunts for would have been visible during row 1's back press, so the row was a recollection rather than a new test — **and the recollection is that it FAILED**. 2026-09-21, reported by the developer: pressing back during a game shows **the app shrinking and minimising** before the confirmation appears. That is precisely the disagreement described above, observed. **It also refutes this note's own earlier conclusion** that the shell-side concern was retired and no `android/` change was indicated — that was an inference from the manifest's silence, and the device overrode it. **Decision 2026-09-21: ACCEPTED as cosmetic, no fix.** The confirmation still appears, the deck is intact and nothing is lost, while the one-attribute opt-out is likely inert (the animated activity is Chrome's, not this shell's) and Bubblewrap exposes no lever for it anyway — so the alternative was a hand-edit to a regenerated file, bought for nothing. It is written into `visual-assets/tester-notes.md` under "expected", because a tester who does not know will file it. **Re-running this row at step 12 is NOT informative** — Play's signature changes the certificate, not the back animation — so unlike rows 1-6 this one does not re-run. Reasoning in `docs/agent_findings.md`._ | Observed and ACCEPTED 2026-09-21 — cosmetic, no fix; documented for testers |

**Row 7 was the one that could send this back to plan 1, and on 2026-09-21 it did — this paragraph
said the opposite for a few hours and was wrong.** The reasoning that failed is worth keeping, because
it is the shape of mistake this section exists to prevent: the generated manifest sets no
`android:enableOnBackInvokedCallback`, that attribute is an opt-**out**, therefore the shell is on the
modern path and nothing in `android/` needs changing. Every clause of that is true and the conclusion
does not follow. **The device says the OS previews the app leaving while the web app handles the
press** — the exact disagreement the row was written to catch.

**The remedy is probably not in `android/` either, and for a reason the row did not anticipate.** The
generated manifest declares only `LauncherActivity`, `FocusActivity`, `WebViewFallbackActivity` and
the delegation service; **the activity that renders the web content belongs to Chrome**, launched into
this app's task but declared in Chrome's manifest. So `android:enableOnBackInvokedCallback="false"` on
this `<application>` governs the launcher shell and not the activity Android is animating, which makes
the one-attribute fix likely inert rather than merely awkward. It is **reasoned, not measured** —
confirming it needs `adb shell dumpsys activity activities` to see which activity is resumed, and no
device has ever enumerated over USB here. Row 1 was the one to run first — it is the whole feature —
and it passed.
...
`swipeIntent` mapping, the `PREVIOUS` reducer case, ArrowLeft end to end through the HUD count, the `custom` value the stack hands `AnimatePresence` for
every index move (2026-09-19), the movement it hands the card's own `movement` prop beside it
(2026-09-21), the `pt-8` the two screens now share, asserted at both ends (2026-09-21), and — since
the step back became a gesture the finger PERFORMS rather than one the release plays (2026-09-21) —
the drag configuration that pins the card, the 1:1 mapping as a pure function, and the progress the
hook hands the card that replaces the peek. What they cannot cover is a service worker, a browser's
download behaviour, a laid-out screen, a stacking order and a thumb.
...
| 2   | **Open the saved PDF and confirm it is the year cards, all 10 pages** — eight of cards, then two of a repeated card back. (This row said 20 until 2026-09-21, when the file was decoded properly.) The repo has still only read structure; nobody in a session has looked at a rendered page.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Pending                                                                                                                                                                                                                                                                                                                                                                      |
...
| 9   | **A step FORWARD throws the card out to the right, from every input.** A right swipe and ArrowRight must both fly the outgoing card off to the RIGHT and uncover the card beneath it — before 2026-09-19 every keyboard advance flew the other way. Also: press → quickly while a card is still leaving; the first card must keep its direction. **And the card must stay ON TOP for the whole flight (2026-09-21)** — the developer's report was that on release it dropped behind the deck and slid out from under its replacement. Root cause was `perspective-distant` making the INCOMING card a stacking context, which beat the popLayout'd outgoing one on tree order; the forward exit now carries `zIndex: 1`. The paint order was measured in headless Chrome on a reduction (`agent_findings.md`, 2026-09-21), but the real swipe has still been felt by no thumb. **And since 2026-09-22 it must take ~781ms rather than 250ms** — the flight was ~3x the left swipe's rate and the developer asked for the two to match, so its duration is now `EXIT_DISTANCE_PX` at the rate the left swipe sets. The whole of the check is whether it reads as deliberate or as slow motion; if it is the latter, the lever is `TRAVEL_SPEED_PX_PER_S` and nothing else. | Pending                                                                                                                                                                                                                                                                                                                                                                      |
| 10  | **A step BACK plays that in reverse, and the returning card lands ON TOP (2026-09-21).** ArrowLeft must bring the card you are going back to IN from off the right edge — not throw the current one out to the left, which is what it used to do and what the developer asked to change. (The SWIPE now performs that entrance with the finger instead; rows 13–15 cover it.) **ArrowLeft now takes the same ~781ms the forward throw does (2026-09-22)** — same 600px, same rate — so confirm the keyboard's two directions read as one journey run each way rather than one being visibly brisker. Two things only a browser can show: the returning card must paint OVER the one it is covering for the whole entrance, and the card being left must not sink behind the deck's preload (it sits at `zIndex: -1`, the preload at `-z-10`). On card 1 a left swipe or ArrowLeft snaps back with no movement at all.                                                                                                                                                                                                                                                                                                                                                     | Pending                                                                                                                                                                                                                                                                                                                                                                      |
| 11  | **Both animations under `prefers-reduced-motion: reduce`.** Set the OS preference. A forward card should fade rather than fly, and a backward card should APPEAR in place rather than crawl in — the one failure to watch for is a returning card stranded off-screen, which is what `{ type: false }` on positional keys is reasoned to prevent and which nothing local can observe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Pending                                                                                                                                                                                                                                                                                                                                                                      |
| 12  | **The logo does not move between the front door and the picker (2026-09-21).** Press the welcome screen's button, then Back, and watch the logo: it must not shift vertically at all. The two `<main>` elements now share `pt-8` and the hero is the first in-flow child on both, so this is arithmetic — but jsdom computes no layout, and the class assertion at each end cannot see a pixel. Check it at 320px too, where the widened Back button (`px-4` since the same date, 8px broader than it was) passes closest to the logo's top-left corner: the text must still clear it. Then confirm the point of the rest of that pass — the picker's Start, its inputs, the "+" and the suggestion cards must read as the same size as the front door's button and step cards, rather than a notch smaller, which is what the developer saw and asked to have fixed.                                                                                                                                                                                                                                                                                                                                                                                                     | Pending                                                                                                                                                                                                                                                                                                                                                                      |
| 13  | **The current card never travels left of its resting position (2026-09-21).** The developer's first instruction, and it is one property: drag left, hard and far, and the card must not move a pixel that way. Then drag RIGHT — it must still give, elastically, as it always has — and slide back to rest without releasing. `dragElastic` is `{ left: 0, right: 0.35 }` against a `left: 0` constraint, and the difference between that and the old scalar `0.35` is a transform jsdom never computes: nothing in the suite would fail if it were put back.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Pending                                                                                                                                                                                                                                                                                                                                                                      |
| 14  | **The previous card follows the thumb one for one, and the release has no seam (2026-09-21).** Drag left slowly from card 2 or later: the card you are going back to must come in from off the RIGHT edge at exactly the speed of the finger, from one card-width out, landing on top of the current card. Drag right again and it must go back out. Then release past the threshold and watch the join — the card must CONTINUE from where your thumb left it, not jump back off-screen and re-run the entrance. **This is the animation the 2026-09-22 speed change is measured FROM**, so it must be unchanged: still 250ms over whatever the thumb left. That jump is the whole reason `entranceFromProgress` exists, and it is the single most likely thing to be wrong here.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Pending                                                                                                                                                                                                                                                                                                                                                                      |
| 15  | **A cancelled step back, and the same under reduced motion (2026-09-21).** Drag left a little and release below the threshold: the previous card must slide back out over ~250 ms while the current card stays put, with no flicker and no horizontal scrollbar appearing at any point in the gesture (the card is parked with `display: none`, which is the only one of the three hiding mechanisms that takes it out of layout). Then set `prefers-reduced-motion: reduce` and repeat — **the slide-out is an imperative `animate()`, so it does NOT read `MotionConfig reducedMotion="user"` and will still slide.** Known and documented in the hook; the row exists to record how it actually feels rather than to fail.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Pending                                                                                                                                                                                                                                                                                                                                                                      |
...
1. **Generate the shell.** `bubblewrap init` against the deployed manifest URL, inside `android/`. At the prompts: the **application id is `aleixrabassa.playlistjitster`** — type it, and do not accept Bubblewrap's host-derived default (`app.vercel.playlistjitster`), which puts the app in a namespace nobody here owns and is **permanent after first publish**. Keep the manifest's name and short name, standalone display, the page colour for theme and background, and the 512 icon as the splash source. Answer the **orientation** prompt with the any/default option — the absence of `orientation` in `src/pwa/manifest.ts` is a decision pinned by a test, and this prompt is the thing that threatens it. **Decline Play Billing and the geolocation permission**, keep the default **Custom Tabs fallback**, and leave the shortcuts list empty.
   ...
   - **What the shell opens on:** `startUrl` is `/`, so every **cold launch shows the welcome screen**, by the 2026-09-18 decision — the flag that skips it is `useState` in `App.tsx` and is deliberately not persisted. Testers should expect it; `visual-assets/tester-notes.md` says so.
