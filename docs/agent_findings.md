...
## 2026-09-24 — Store assets: the HUD leaks the Spotify title, the 512 icon is a palette PNG, and Canva's quota is small

Produced the Play listing graphics and screenshots (`docs/store/assets/`, inventory in
`docs/store/listing.md` §4a) with Playwright against the deployed app and Canva. What was learned:

- **The 2026-09-19 relabel fixed the picker, not the game screen.** The HUD renders the playlist's REAL
  Spotify title via `deckLabel()`, and "Jitster official"'s real title is **"Hitser"**. Any game-screen
  screenshot of that deck puts a near-copy of the mark in the listing. The capture script uses
  "Rock Party" for that reason; renaming the Spotify playlist (the developer owns it) is the durable fix.
- **`public/pwa-512x512.png` is a palette PNG (PIL mode `P`)**, not 32-bit. Harmless for the PWA, but
  Play asks for a 32-bit PNG, so `assets/graphics/play-icon-512.png` is an RGBA regeneration from the
  master. First time a check in this repo opened that file.
- **Playwright's PNG screenshots are RGBA; Play wants no alpha.** The script writes JPEG q95 instead.
- **Playwright is run from a scratch dir, never added to `devDependencies`** (pnpm-only rule, same
  precedent as `@bubblewrap/cli`). `docs/store/screenshots.mjs` documents the invocation. A
  `sed`-then-Python edit once turned a regex `\b` into a literal backspace (0x08) inside the script,
  which made the reveal filter silently match nothing — check with `od -c` if a filter "never fires".
- **Canva MCP limits, measured:** `create-design` generation hit `quota_exceeded` after three designs in
  one session, and `resize-design` reported one remaining trial use after the first. Both are
  avoidable: `edit-design`'s `add_page` accepts its own `width`/`height`, so one design can hold pages
  of different sizes (all five social formats live in one design) and editing costs no generation
  quota. `create-design`'s `format` did NOT honour "Custom 1024x500 px" (it came back 1264×1264).
- **Canva-generated backgrounds can carry faint baked copies of their headline text** — replacing the
  text element leaves a ghost. Cover with a solid shape layered to the back, or keep the original text.
- **Local assets reach Canva by `create-upload-url` + a raw-bytes POST**; `upload-asset-from-url`
  cannot read `docs/assets/logo.png` because it is deliberately not deployed.

## 2026-09-24 — Store copy and visual assets moved out of `docs/` into `visual-assets/`

At the developer's request, `docs/assets/` → `visual-assets/assets/` (the 1254 logo master) and
`docs/store/` → `visual-assets/` (`listing.md`, `tester-notes.md`, `screenshots.mjs` and the
store `assets/`). Every live reference was rewritten — AGENTS.md, README, architecture, development,
the plans, and two comments in `LandingScreen.tsx`/`.test.tsx`. **Entries above this one in this file
still say `docs/assets` and `docs/store`**: they are history and were left as written, so read those
paths as the old names. The reason the master is kept out of `public/` is unchanged by the move.

`visual-assets/assets/play-tablet-chromebook/` is **git-ignored** by the same request — the tablet
and Chromebook captures exist only locally and are regenerated by `visual-assets/screenshots.mjs`.

## 2026-09-24 — "Hitser" fixed in code; the 512 icon deliberately left as a palette PNG

**The HUD fix.** `src/game/playlist-display-name.ts` holds an id → label map (one row: the first
suggestion → "Jitster official") and `playlistDisplayName()`. It is read by `deckLabel()` (HUD, end-screen
count line, PDF filename, new library saves), by `EndScreen`'s multi-playlist list (which bypasses
`deckLabel`), and by `loadLibrary`, which rebuilds a saved name from `ids[0]`'s override plus the same
"+N more" count. **Applied on read, never on write**, because a stored session keeps the fetched name
in `GameState.playlists` and a library entry holds a finished label string — only a read-time lookup
heals data saved before the fix, without a migration. The map is in `src/game/` so the pure modules need
not import a component; `SUGGESTED_PLAYLISTS[0]` now reads its label from it. Only rows labelled for the
APP belong in the map — the other suggestions' labels are renderings of their Spotify titles, and
overriding those would make the HUD disagree with what the QR opens. The trademark guard in
`LandingScreen.test.tsx` widened from `/hitster/i` to `/hitst?er/i`, which also catches "Hitser".
**Not fixable from this repo:** the Spotify app still shows "Hitser" when a QR is scanned; renaming the
playlist on Spotify is the developer's action.

**The icon.** `public/pwa-512x512.png` stays a palette PNG on purpose: the regeneration rule keeps the
smaller of RGB and 256-colour palette, and the RGBA version is ~173 kB against 67.5 kB — ~105 kB more in
every install's precache for no PWA benefit. Bubblewrap re-fetches `iconUrl` for launcher mipmaps, which
do not need 32-bit input. Only the Play Console store icon does, and that is
`visual-assets/assets/graphics/play-icon-512.png`, uploaded by hand.

## 2026-09-24 — `visual-assets/` flattened; the logo master is no longer tracked

Second restructure the same day, at the developer's request, superseding the paths in the entry above
that moved things out of `docs/`: `visual-assets/store/*` moved up to `visual-assets/` (so it is now
`visual-assets/listing.md`, `tester-notes.md`, `screenshots.mjs` and `assets/`), `store/` is gone, and the
logo master moved to **`visual-assets/logo-master/logo.png`, which is git-ignored** — it was renamed
first because the store's own `assets/` folder would otherwise have merged into the excluded one. The
committed store outputs are `visual-assets/assets/{graphics,play-phone,social}`; `play-tablet-chromebook/`
