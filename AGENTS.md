...
| [`docs/plans/plan.phase-8-look-and-shell.md`](./docs/plans/plan.phase-8-look-and-shell.md) | Phase 8, plan 1 — neon-ring card design, contrast re-audit, PWA, icon set. **Not built**                    |
...
**Do not build ahead of the current phase.** The plan defers things deliberately. Current phase: **8, partly built.** Phases 1–7 are complete; the app is playable end to end, has a design surface, and fails legibly. `src/App.tsx` is the **real container** and the only caller of `useGameSession()`. Of Phase 8's three plans, **plan 2 ([`plan.phase-8-features.md`](./docs/plans/plan.phase-8-features.md)) is built** — the shareable deck URL, the saved-playlist library, the printable PDF export, and the audio reversal below — and **plan 3 is resolved as won't-build**. **Plan 1 ([`plan.phase-8-look-and-shell.md`](./docs/plans/plan.phase-8-look-and-shell.md)) has not been started**: the neon-ring card design, the contrast re-audit, the PWA and the icon set are all still open, and it is the plan to pick up next. Note that plan 2 depended on it only **softly** and did not wait — so the PDF's print palette is deliberately its own and will not change when the screen is redesigned.
